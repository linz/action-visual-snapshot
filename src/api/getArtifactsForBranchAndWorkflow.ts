...
export async function getArtifactsForBranchAndWorkflow(
  octokit: Octokit,
  { owner, repo, workflow_id, branch, commit, artifactName }: GetArtifactsForBranchAndWorkflow,
  ...
  for await (const response of octokit.paginate.iterator(octokit.actions.listWorkflowRuns as any, {
    owner,
    repo,
    ...
    const workflowRunsForCommit = commit
      ? workflowRuns.filter((run: (typeof workflowRuns)[number]) => run.head_sha === commit)
      : workflowRuns;
